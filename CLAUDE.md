...
| 195 | 팀배틀 중 클래식 PvE 레이드 유입 버그 수정 | `updateIdleRaid()` 첫줄에 `gameMode === "teambattle"` 가드 추가 — SPECIAL FORCES 토글(`specialForcesEnabled`)이 어떤 경로로든 켜진 채 10초 이상 정지하면 팀배틀 중에도 클래식 몬스터(`enemies[]`)가 스폰되고, 그 몬스터의 근접 공격 판정이 `gameMode` 체크 없이 `state="dead"`(클래식 게임오버)로 강제 전환시켜 팀배틀 리스폰(`playerRespawnTimer`) 로직을 건너뛰는 문제 발견 → 팀배틀에서는 레이드 자체가 아예 발동 못 하도록 원천 차단 |

**테스트 방법 및 결과 (2026-07-01):** 온라인 배틀(8인 실제 접속)은 이 세션 네트워크 정책상 PeerJS CDN(unpkg.com) 및 배포 주소 자체가 차단되어 불가능 — 대신 오프라인 팀배틀(AI 봇 채움)을 모바일 터치 이벤트로 재현하여 4캐릭터(HAWK/BULL/VIPER/MEDIC) 전원의 이동/사격(전 무기)/무기교체/스킬/폭탄을 개별 검증, 캐릭터당 30초 시뮬레이션 스트레스 테스트, 금고파괴→시네마틱→수송차 이동(승/패 양쪽)→미션종료 버튼, 파워오브젝티브 실제 점령, 리스폰 사이클, BULL C4 설치/폭파, 미니맵 렌더링까지 스크린샷 기반으로 확인. 위 버그 1건 발견 후 수정 완료, 재검증 통과.

| 196 | 팀배틀 맵 전면 재설계 (Overwatch Escort / CS·Valorant 참고) | `tbMapLayout()` 신설 — 모든 팀배틀 함수(맵/금고/스폰/터렛/AI/크레이트/수송차경로/파워오브젝티브)가 이 함수 하나만 참조하도록 통일. 맵 크기 3300×2400→**3600×2000**(팀배틀 전용, 클래식 모드는 `resetWorld()`에서 3300×2400로 복원). 레인 4개→**3개**(상/중/하)로 단순화하고 기지벽 도어도 3개로 축소, 중앙은 막힌 십자 요새 대신 **6기둥 링 형태의 개방형 플라자**(파워오브젝티브 쟁탈전용)로 교체. 금고는 각 팀 본진 안쪽 깊숙이(중앙 레인 라인) 배치, 스폰 플랫폼은 금고와 기지벽 사이, 터렛은 금고 상/하 플랭크 방어. 레인 중간에 CS/Valorant식 코너 L엄폐, 상↔중·중↔하 레인 사이 이동을 유도하는 사선 엄폐 추가 |
| 197 | 팀배틀 맵 재설계에 따른 연쇄 업데이트 | `computeConvoyRoute()`(3레인+지그재그 2종=5경로로 축소), `setupAiPlayers()`/`updateAiPlayers()`(AI 스폰·리스폰 좌표), `setupCrates()`(플라자·레인 엄폐 근처 12개 재배치), `pickNeutralObjSpot()`(marginX 900→750, marginY 160→250), `startVaultCinematic()`의 시네마틱 카메라 경로, 온라인 배틀(`startTBOnlineGame`/`applyTBStart`/`setupAiPlayersOnline`)의 금고·스폰 좌표까지 전부 `tbMapLayout()` 기준으로 일치시킴 |
| 198 | 도어 높이 축소로 인한 convoy 벽 끼임 버그 발견 및 수정 | 기지벽 도어 높이를 130px로 좁혔다가, A* 그리드 margin(52px)×2 + 그리드 셀(64px) 크기 때문에 문틈 셀이 "막힘"으로 오판되어 수송차가 문 앞에서 영구 정지하는 문제 발견(item #189와 동일 계열 버그). 도어 높이를 **200px**로 늘려 해결. 40회 랜덤 경로 검증 + 실제 승리/패배 시나리오 6회 연속 재현으로 재검증 통과 |

**재설계 테스트 방법 및 결과 (2026-07-01):** 새 맵에서 전체 자동화 테스트 재실행 — 스폰 위치 벽 충돌 검사(플레이어/AI 5명/금고/스폰플랫폼/터렛 4개 전부 통과), 수송차 5경로 랜덤 생성 40회(양방향) 전부 유효, 실제 금고파괴→시네마틱→수송차 이동 승리 시나리오 6회 연속 성공(경로 길이 51~70 다양하게 샘플링됨), 패배 시나리오(수송차 파괴) 정상 종료, 4캐릭터 이동/사격/스킬/폭탄 + 30초 스트레스 테스트, 파워오브젝티브 실제 점령, BULL C4, 미니맵/플라자 렌더링까지 스크린샷 기반 확인.

| 199 | convoy 도어 높이 축소로 인한 재발 버그 완전 진단 및 수정 | 실제 폰 플레이 녹화에서 수송차가 ROUTE 5%에서 16초+ 정지하는 것을 프레임 추출로 확인 → 10가지 경로×방향 조합을 `Math.random()` 강제 오버라이드로 결정적 재현하는 전용 테스트 하니스 작성. 근본 원인 3가지를 순차 발견: ①`computeConvoyRoute()`가 경유점 세그먼트 하나가 실패해도 조용히 건너뛰고 이어붙여, 서로 먼 두 지점이 waypoints에서 바로 이웃하게 되고 convoy가 그 사이를 직선으로 가로지르다 장애물에 영구 정지 — "세그먼트 하나라도 실패하면 즉시 전체를 직행 경로로 대체"하도록 수정 ②중앙 플라자 6기둥이 정확히 0°/180°(중앙 레인의 수평선) 위에 배치되어 그 라인 자체가 A*로도 뚫리지 않음 — 기둥 배치를 30도 회전해 수평/수직축을 피함 ③레인 입구 기둥이 레인 중심선 위에 그대로 있어서, 도어의 margin(52px) 때문에 줄어든 "빈 행"과 기둥이 막는 "행 범위"가 겹쳐 도어를 나선 직후 사방이 막히는 chokepoint 발생(도어를 넓혀도 기둥이 그 행을 다시 덮어서 해결 안 됨) — 기둥을 레인 중심선에서 세로로 140px 띄워서 해결. 부가로 레인 입구 기둥(6개)·플라자 기둥(6개)·중앙 레인 기둥(2개)의 thin(margin 6px) 태그를 solid(margin 52px)로 전환 — convoy(r=35) 스케일에 thin margin이 근본적으로 안전하지 않음이 이번 조사로 확인됨. 10가지 경로×방향 조합 전부 재검증 통과 + 40회 랜덤 시뮬레이션 + 4캐릭터 회귀 테스트까지 재확인 |

| 200 | 팀배틀 8인(4v4) 대응 맵/게임 전면 개편 | 맵 3600×2000→**4000×2400**. 레인별 캐릭터 테마: **상단=저격 골목**(VIPER — 레인 라인 끝까지 개방+측면 엄폐만, CS 미드 참고), **하단=근접전 구역**(BULL — 밀집 블록·모퉁이 다수, 배틀필드 락커 참고), **중앙=수송차 간선+플라자**(r 260→300). 레인 사이 **회전 통로** 4개(Valorant 커넥터식 세로벽) 추가. **금고를 중앙 레인에서 상/중 레인 사이(`vaultCY`)로 이동** — 방어팀이 두 출입구를 커버해야 하는 CS 폭탄사이트식. 오프라인 AI도 **4v4**(적 4명 전 캐릭터+아군 3명)로 온라인 8인과 동일 구성 |
| 201 | 방어 터렛 2→3기/팀, 데미지 하향 | `setupTurrets()` — 금고 위/아래 2기 + 하단 레인(금고 반대편 약점) 1기. 단발 데미지 42→**28**(`turret.dmg` 필드화), HP 400→**450**, 사거리 520→560 |
| 202 | 신규 아이템 3종 + 고정 스폰 패드 (Quake/Halo식 타이머 아이템 컨트롤) | `itemSpawners[]`(6개소)+`setupItemSpawners()`/`updateItemSpawners()` — **SPEED**(이속 +30% 8초, 저격 골목 한복판 ×2 — 하이리스크), **ENERGY**(스킬게이지 +50%, 플라자 대각 ×2), **SHIELD**(받는 피해 50% 감소 8초, 근접전 구역 ×2). 15~18초 리스폰, 빈 패드도 회전 점선 링으로 위치 표시(`drawItemSpawnerPads`). 효과 적용: 이동속도(`speedBoostTimer`), AI사격/터렛탄/적폭탄 데미지(`shieldTimer`), `drawPickups()` 타입별 아이콘(셰브론/번개/방패) |
| 203 | 수송차 속도 55→66 | 맵 확장 비율에 맞춰 상향 — 최장 경로(하단 관통/지그재그)도 에스코트 페이즈 ~80초 내 완주 |

**8인 대응 개편 테스트 (2026-07-02):** 전 레인·금고·스폰 연결성 11종 경로 전부 유효, 스폰 충돌 검사(플레이어+AI 7+터렛 6+금고+플랫폼) 전부 통과, 아이템 3종 획득 효과·15초 리스폰·타이머 감쇠 실측 확인, 수송차 10조합(5경로×양방향) 결정적 재현 끼임 0건, 최장 경로 양방향 완주 확인, 4캐릭터 조작/스킬/폭탄 회귀 통과, 구역별 스크린샷(플라자 8인 교전/저격 골목/근접전 구역/금고+터렛) 확인.
