...
function drawMinimap() {
  const w = 156, h = 104, x = width - w - 18, y = 16;
  ...
  for (const agent of game.agents) {
    if (agent.hp <= 0 || (!agent.controlled && !canSee(player, agent))) continue;
    ctx.fillStyle = agent.team === "blue" ? "#7fd0ff" : "#ff554e";
...
function nearestVisibleEnemy(agent) {
  ...
  for (const other of game.agents) {
    if (other.hp <= 0 || other.team === agent.team || !canSee(agent, other)) continue;
...
function pointInWall(x, y) { return game.walls.some((w) => pointInRect(x, y, w)); }
function pointInRect(x, y, r) { return x >= r.x && x <= r.x + r.w && y >= r.y && y <= r.y + r.h; }
function inGrass(agent) { return game.grass.some((g) => pointInRect(agent.x, agent.y, g)); }
function emitPing(x, y, radius, color, life) { game.pings.push({ x, y, radius, color, age: 0, life }); }

function muzzle(agent) {
  for (let i = 0; i < 7; i++) {
    const a = agent.angle + Math.PI + (Math.random() - 0.5) * 0.8;
    const s = 50 + Math.random() * 160;
    game.sparks.push({ x: agent.x + Math.cos(agent.angle) * 34, y: agent.y + Math.sin(agent.angle) * 34, vx: Math.cos(a) * s, vy: Math.sin(a) * s, size: 1.5 + Math.random() * 2.5, color: agent.team === "blue" ? "#7fd0ff" : "#ff554e", life: 0.18, maxLife: 0.18 });
  }
}
...
function impact(x, y, color) {
  for (let i = 0; i < 10; i++) {
    ...
    game.sparks.push({ x, y, vx: Math.cos(a) * s, vy: Math.sin(a) * s, size: 1.5 + Math.random() * 3, color, life: 0.28, maxLife: 0.28 });
  }
}
...
window.addEventListener("keydown", (e) => { keys.add(e.key.toLowerCase()); if (e.key === " " || e.key === "Shift") e.preventDefault(); });
...
canvas.addEventListener("pointermove", (e) => {
  ...
  } else { mouse.x = x; mouse.y = y; mouse.active = true; }
});
canvas.addEventListener("pointerdown", (e) => {
  ...
  } else { mouse.x = x; mouse.y = y; mouse.active = true; mouse.down = true; }
});
canvas.addEventListener("pointerup", (e) => {
  if (e.pointerId === joystick.id) { joystick.active = false; joystick.id = null; joystick.dx = 0; joystick.dy = 0; }
...
document.querySelector("#reload-btn")?.addEventListener("pointerdown", (e) => { e.stopPropagation(); if (game.running) reload(player); });
